...
function getDetailTabs(selectedType) {
  ...
  if (selectedType === "trip") {
    return [{
      ...
      key: "progress",
      label: "履约进度",
      keys: ["current_step_index", "total_step_count", "distance_traveled_km", "distance_remaining_km", "time_elapsed", "arrival_execution_result"]
    }, {
      key: "route",
      label: "路径关联",
...
function renderServiceOrderActions(row, actions) {
  ...
  if (row.order_status === "WAITING_CUSTOMER_CONFIRM") {
    return /*#__PURE__*/React.createElement(RowActionButton, {
      onClick: () => actions.confirmServiceOrder(row.service_order_id)
    }, "\u5BA2\u6237\u786E\u8BA4");
  }
