...
function App() {
  ...
  function createDeploymentTasks() {
    ...
    appendTaskPlanningAudit(planningResults.map(item => item.planningRun), planningResults.map(item => item.planningResult));
  ...
  function advanceRouteExecution(routeExecutionId) {
    ...
    const batteryDeltaPercent = robotaxi?.battery_capacity_kwh ? Number((batteryDeltaKwh / robotaxi.battery_capacity_kwh * 100).toFixed(2)) : robotaxi?.max_range_km ? Number((distanceDeltaKm / robotaxi.max_range_km * 100).toFixed(2)) : 0;
...
function isFleetOperationWorkActiveStatus(row) {
  return ["CLEANING_IN_PROGRESS", "MAINTENANCE_IN_PROGRESS", "CHARGING", "READY_TO_DISCONNECT", "DIAGNOSING"].includes(row?.task_status);
}
