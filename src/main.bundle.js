...
function RecordTable({
  ...
  }, isTripPage ? "履约执行事件" : isDemandSimulationStrategyPage ? "需求模拟执行" : isRoutePlanningPage ? "路径规划执行记录" : isPricingPage ? "定价执行记录" : isOrderMatchingPage ? "匹配执行记录" : "最近任务事件"), /*#__PURE__*/React.createElement(Table, {
    size: "small",
    rowKey: eventRowKey,
...
function getDetailTabs(selectedType) {
  ...
  if (selectedType === "serviceOrder") {
    return [{
      ...
      key: "pricing",
      label: "报价结算",
      keys: ["price_estimation_route_id", "estimated_pricing_decision_id", "final_pricing_decision_id", "quote_base_fare", "quote_distance_unit_price", "quote_time_unit_price", "estimated_distance_km", "estimated_duration_min", "estimated_price", "quoted_price", "actual_distance_km", "actual_duration_min", "final_price", "pricing_explanation", "pricing_breakdown_snapshot"]
    }, {
      key: "matching",
      label: "匹配履约",
  ...
  if (selectedType === "trip") {
    return [{
      ...
      key: "route",
      label: "路径关联",
      keys: ["route_id", "route_planning_run_id", "route_summary", "route_detail", "route_history"]
    }, {
      key: "time",
      label: "状态时间",
...
function renderServiceOrderActions(row, actions) {
  ...
  if (row.order_status === "SETTLING") {
    return /*#__PURE__*/React.createElement(RowActionButton, {
      onClick: () => actions.settleServiceOrder(row.service_order_id)
    }, "\u7ED3\u7B97");
  }
...
function createPricingStrategyRows(data, pricingStrategyRuns) {
  ...
  return (data.pricingStrategies || []).map(strategy => ({
    ...strategy,
    dynamic_multiplier: Number((strategy.supply_demand_multiplier * strategy.time_period_multiplier * strategy.service_area_multiplier * strategy.channel_multiplier).toFixed(3)),
