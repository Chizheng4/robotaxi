...
const deploymentStatusOptions = ["WAITING_START", "MOVING", "ARRIVED", "ARRIVAL_ABNORMAL", "COMPLETED", "CANCELLED", "FAILED"];
const routeExecutionStatusOptions = ["WAITING_ROUTE", "MOVING", "ARRIVED", "ARRIVAL_ABNORMAL", "PAUSED", "COMPLETED", "CANCELLED"];
...
function App() {
  ...
  function settleServiceOrder(serviceOrderId, visibleOrder = null) {
    console.info("[settlement-debug] click", {
      serviceOrderId,
      visibleOrderStatus: visibleOrder?.order_status
    });
    const serviceOrder = serviceOrders.find(item => item.service_order_id === serviceOrderId);
    if (!serviceOrder) {
      console.info("[settlement-debug] stop:no-service-order", {
        serviceOrderId
      });
      return;
    }
    ...
    const canSettle = settlementOrder.order_status === serviceOrderTypes.ServiceOrderStatus.SETTLING || [tripTypes.TripStatus.ARRIVED_DESTINATION, tripTypes.TripStatus.SETTLING, tripTypes.TripStatus.COMPLETED].includes(trip?.trip_status);
    ...
    if (!canSettle) {
      console.info("[settlement-debug] stop:not-settleable", {
        serviceOrderId
      });
      return;
    }
...
function RecordTable({
  ...
  }, isTripPage ? "履约行驶事件" : isDemandSimulationStrategyPage ? "需求模拟执行" : isRoutePlanningPage ? "路径规划执行记录" : isPricingPage ? "定价执行记录" : isOrderMatchingPage ? "匹配执行记录" : "最近任务事件"), /*#__PURE__*/React.createElement(Table, {
    size: "small",
    rowKey: eventRowKey,
  ...
  function getActionColumn() {
    ...
    if (isServiceOrderPage) {
      return {
        ...
        fixed: "right",
        width: 150,
        render: (_, row) => renderServiceOrderActions(row, {
          ...actions,
          page,
          objectType,
          idField
    ...
    if (isTripPage) {
      return {
        ...
        fixed: "right",
        width: 150,
        render: (_, row) => renderTripActions(row, {
          ...actions,
          page,
          objectType,
          idField
...
function getDetailTabs(selectedType) {
  ...
  if (selectedType === "trip") {
    return [{
      ...
      key: "location",
      label: "服务位置",
      keys: ["pickup_service_area_id", "pickup_cell_id", "pickup_location_summary", "pickup_location_detail", "dropoff_service_area_id", "dropoff_cell_id", "dropoff_location_summary", "dropoff_location_detail", "current_cell_id", "current_location_summary", "current_location_detail"]
    }, {
      key: "progress",
      label: "履约进度",
      keys: ["current_step_index", "total_step_count", "distance_traveled_km", "distance_remaining_km", "time_elapsed", "arrival_execution_result", "exception_type"]
    }, {
      key: "route",
      label: "路径关联",
...
function createTripRoute(trip, data, options = {}) {
  const originCellId = options.originCellId || trip.current_cell_id;
  const targetCellId = options.targetCellId;
  const targetServiceAreaId = options.targetServiceAreaId;
...
function createPriceEstimationRoute(serviceOrder, data, routePlanningRunId) {
  ...
  const pickupPlan = originCellId === pickupCellId ? {
    ...
    route_steps: createSingleCellRouteSteps(data, originCellId)
  ...
  const roadSegmentSequence = [...pickupPlan.road_segment_sequence, ...dropoffPlan.road_segment_sequence].filter((segmentId, index, list) => segmentId && list.indexOf(segmentId) === index);
  const totalDistance = Math.max(0, routeSteps.length - 1) * (data.maps[0]?.cell_size_m || 50);
  ...
  return {
    ...
    route_segments: [{
      segment_type: "CUSTOMER_TO_PICKUP",
      origin_cell_id: originCellId,
      target_cell_id: pickupCellId,
      step_count: Math.max(0, pickupPlan.route_steps.length - 1)
    }, {
      segment_type: "PICKUP_TO_DROPOFF",
      origin_cell_id: pickupCellId,
      target_cell_id: targetCellId,
      step_count: Math.max(0, dropoffPlan.route_steps.length - 1)
    }],
    map_id: data.maps[0].map_id,
    start_cell_id: originCellId,
    end_cell_id: targetCellId,
...
function runFinalFareCalculation({
  strategy,
  serviceOrder,
  ...
  const hasQuotedPriceOnly = Number.isFinite(quotedPrice) && quotedPrice > 0 && serviceOrder.quote_base_fare == null && serviceOrder.quote_distance_unit_price == null && serviceOrder.quote_time_unit_price == null && pricingSnapshot.base_fare == null && pricingSnapshot.distance_unit_price == null && pricingSnapshot.time_unit_price == null;
  ...
  const failureReason = !hasStrategy ? pricingTypes.PricingFailureReason.PRICING_STRATEGY_NOT_FOUND : !hasPricingInputs ? pricingTypes.PricingFailureReason.PRICING_CONFIG_MISSING : actualDistanceKm <= 0 && !hasQuotedPriceOnly ? pricingTypes.PricingFailureReason.INVALID_DISTANCE : actualDurationMin < 0 ? pricingTypes.PricingFailureReason.INVALID_DURATION : null;
...
function getNextTripMovementState(trip, data) {
  ...
  const nextStatus = reachedTarget ? trip.trip_status === tripTypes.TripStatus.ON_THE_WAY_PICKUP ? tripTypes.TripStatus.WAITING_CUSTOMER_BOARDING : tripTypes.TripStatus.ARRIVED_DESTINATION : trip.trip_status;
...
function createTripRouteUpdate(trip, nextTrip, data) {
  ...
  return {
    ...
    nextTrip: {
      ...
      route_history: [...(Array.isArray(trip.route_history) ? trip.route_history : []), createRouteHistoryEntry(route, routeRequest.routeChangeReason, null)],
      current_step_index: 0,
