...
const pageGroups = [{
  ...
  children: [{
    ...
    key: "supplyProductionProfiles",
    label: "供应生产画像"
  }, {
    key: "demandForecastManagement",
    label: "需求预测",
    children: [{
      key: "longTermDemandForecastStrategies",
      label: "预测策略"
    }, {
      key: "longTermDemandForecastRuns",
      label: "预测执行"
    }, {
      key: "longTermDemandForecasts",
      label: "预测结果"
    }]
  }]
}, {
  ...
  children: [{
    ...
    label: "车队分配策略",
    children: [{
      key: "fleetAllocationStrategies",
      label: "分配策略配置"
    }, {
      key: "fleetAllocationRuns",
      label: "分配策略执行"
    }, {
      key: "fleetAllocationResults",
      label: "分配策略结果"
    }]
  }, {
    key: "robotaxiDeliveryOrders",
    label: "Robotaxi 交付"
  }, {
    key: "supplyOrders",
    label: "供给单"
  ...
  children: [{
    ...
    children: [{
      ...
      label: "运维策略管理",
      children: [{
        key: "fleetOperationPolicies",
        label: "运维策略配置"
      }, {
        key: "fleetOperationPolicyRuns",
        label: "运维策略执行"
      }, {
      ...
      children: [{
        ...
        key: "fleetOperationDispatchDecisions",
        label: "调度策略结果"
      }]
    }, {
      ...
      label: "任务规划策略",
      children: [{
        key: "robotaxiTaskPlanningStrategies",
        label: "规划策略配置"
      }, {
        key: "robotaxiTaskPlanningRuns",
        label: "规划策略执行"
      }, {
        key: "robotaxiTaskPlanningResults",
        label: "规划策略结果"
      }]
    }]
  }]
...
const hiddenWorkspacePages = new Set(["simulationEvents", "costCalculationRuns", "revenueCalculationRuns", "costModelProfiles", "taskPriorityConfig", "taskDispatchStrategies", "taskDispatchRuns", "taskDispatchResults", "placeDemandProfiles", "serviceAreaDemandProfiles", "zoneDemandProfiles"]);
const tableConfig = {
  maps: {
    title: "地图管理",
...
function App() {
  ...
  const allFleetOperationTasks = useMemo(() => [...cleaningTasks, ...chargingTasks, ...maintenanceTasks, ...failureHandlingTasks, ...retirementTasks], [chargingTasks, cleaningTasks, failureHandlingTasks, maintenanceTasks, retirementTasks]);
  ...
  useEffect(() => {
    ...
    loadPersistedRuntimeSnapshot().then(snapshot => {
      ...
      setTaskDispatchStrategies(Array.isArray(snapshot.taskDispatchStrategies) && snapshot.taskDispatchStrategies.length ? snapshot.taskDispatchStrategies : taskDispatchStrategyService.initializeDefaultTaskDispatchStrategies());
      setRobotaxiTaskPlanningStrategies(Array.isArray(snapshot.robotaxiTaskPlanningStrategies) && snapshot.robotaxiTaskPlanningStrategies.length ? snapshot.robotaxiTaskPlanningStrategies : robotaxiTaskPlanningService.initializeDefaultRobotaxiTaskPlanningStrategies());
  ...
  function runRepeatedManualBusinessAction(action, {
    maxIterations = 50,
    isProgressResult = null
  } = {}) {
    return runManualBusinessAction(({
      state,
      runtime
    }) => {
      ...
      for (let index = 0; index < maxIterations; index += 1) {
        ...
        workingState = {
          ...
          data: {
            ...(workingState.data || {}),
            ...(result.updates.robotaxis ? {
              robotaxis: result.updates.robotaxis
            } : {}),
            ...(result.updates.workers ? {
              workers: result.updates.workers
            } : {}),
            ...(result.updates.routes ? {
              routes: result.updates.routes
            } : {}),
            ...(result.updates.serviceOrders ? {
              serviceOrders: result.updates.serviceOrders
            } : {}),
            ...(result.updates.trips ? {
              trips: result.updates.trips
            } : {}),
            ...(result.updates.deploymentTasks ? {
              deploymentTasks: result.updates.deploymentTasks
            } : {}),
            ...(result.updates.readinessTasks ? {
              readinessTasks: result.updates.readinessTasks
            } : {}),
            ...(result.updates.routeExecutions ? {
              routeExecutions: result.updates.routeExecutions
            } : {})
          }
        };
      }
  ...
  useEffect(() => {
    ...
    const eligibleRuns = simulationRuns.filter(run => run.simulation_status === "COMPLETED").filter(run => !autoMetricCalculationRunIdsRef.current.has(run.simulation_run_id)).filter(run => isFinanceCalculationTerminal(run.cost_calculation_status)).filter(run => isFinanceCalculationTerminal(run.revenue_calculation_status));
    if (!eligibleRuns.length) return;
    eligibleRuns.forEach(run => autoMetricCalculationRunIdsRef.current.add(run.simulation_run_id));
    window.setTimeout(() => {
      runMetricCalculation(metricPeriodType, {
        automatic: true
      });
    }, 0);
  ...
  function runMetricCalculation(periodType = metricPeriodType, {
    automatic = false
  } = {}) {
    if (metricCalculationInProgress) return;
    ...
    setTimeout(() => {
      try {
        ...
        setMetricCalculationRuns(current => [{
          ...
          error_count: 1,
          calculation_errors: [{
            error_type: "METRIC_CALCULATION_FAILED",
            error_message: error.message
          }],
  ...
  function startProductionBatch(row) {
    ...
    const result = businessPlanningService.startProductionBatch({
      productionBatch: row,
      context: {
        now
      }
  ...
  return /*#__PURE__*/React.createElement(Layout, {
    ...
    footer: [/*#__PURE__*/React.createElement(Button, {
      key: "cancel",
      onClick: () => setFleetOperationPolicyModalOpen(false)
    }, "\u53D6\u6D88"), /*#__PURE__*/React.createElement(Button, {
      key: "save",
      type: "primary",
      onClick: saveFleetOperationPolicy
    }, "\u4FDD\u5B58\u914D\u7F6E")]
  }, /*#__PURE__*/React.createElement("div", {
    className: "timing-rule-editor"
  }, /*#__PURE__*/React.createElement(Descriptions, {
    ...
    label: "\u7B56\u7565\u7F16\u53F7"
  }, pendingFleetOperationPolicy?.fleet_operation_policy_id || "无"), /*#__PURE__*/React.createElement(Descriptions.Item, {
    label: "\u7B56\u7565\u7C7B\u578B"
  }, getDisplayValue(pendingFleetOperationPolicy?.policy_type, "policy_type")), /*#__PURE__*/React.createElement(Descriptions.Item, {
    label: "\u76EE\u6807\u4EFB\u52A1"
  }, getDisplayValue(pendingFleetOperationPolicy?.target_task_type, "task_type"))), Object.entries(fleetOperationPolicyDraft).map(([key, value]) => /*#__PURE__*/React.createElement("label", {
    key: key
  }, /*#__PURE__*/React.createElement("span", null, getFieldLabel(key)), /*#__PURE__*/React.createElement(Input, {
    size: "small",
    type: typeof value === "number" ? "number" : "text",
    value: value,
    onChange: event => setFleetOperationPolicyDraft(draft => ({
      ...draft,
  ...
  function createManualTask() {
    const result = runRepeatedManualBusinessAction(params => businessActionService.createReadinessTask({
      ...params,
      runtime: {
        ...params.runtime,
        context: {
      ...
      maxIterations: data.robotaxis.length,
      isProgressResult: item => item.resultType === "READINESS_CREATED"
    });
    ...
    setTaskEventLogs(logs => [...created.map(item => createEventLog({
  ...
  function getActiveTaskPriorityConfig() {
    return robotaxiTaskPlanningService?.resolveTaskPriorityConfig(getActiveRobotaxiTaskPlanningStrategy()) || taskPriorityConfigs[0] || robotaxiTaskPriorityService.initializeDefaultPriorityConfig();
  }
  ...
  function runFleetOperationPolicy(taskType) {
    const policy = fleetOperationPolicies.find(item => item.target_task_type === taskType && item.policy_status === "ACTIVE") || fleetOperationPolicies.find(item => item.target_task_type === taskType);
    if (!policy) {
    ...
    if (result.tasks?.length) {
      ...
      setTaskEventLogs(logs => [...result.tasks.map(task => createEventLog({
        event_type: taskTypes.TaskEventType.TASK_CREATED,
        event_result: taskTypes.TaskEventResult.SUCCESS,
  ...
  function createFleetOperationRouteExecution(task, robotaxiOverride = null) {
    ...
    if (result.succeeded) {
      ...
      setRouteExecutions(items => items.some(item => item.route_execution_id === result.routeExecution.route_execution_id) ? items.map(item => item.route_execution_id === result.routeExecution.route_execution_id ? result.routeExecution : item) : [result.routeExecution, ...items]);
  ...
  function startFleetOperationWork(task) {
    ...
    const robotaxi = operationalData.robotaxis?.find(r => r.robotaxi_id === task.robotaxi_id);
    if (!robotaxi) return;
    const result = fleetOperationTaskService.startFleetOperationWork({
      task,
      robotaxi,
      context: {
        now,
  ...
  function completeFleetOperationWork(task) {
    ...
    updateFleetOperationTask(result.task);
    if (Array.isArray(result.costRecords)) setCostRecords(result.costRecords);
    const activation = result.task.task_status === "COMPLETED" ? fleetOperationTaskService.activateNextQueuedFleetOperationTask({
    ...
    setTaskEventLogs(logs => [createEventLog({
      ...
      source_page: collectionKey,
      robotaxi_id: result.task.robotaxi_id,
      message: result.task.task_status === "COMPLETED" ? activation?.activated ? "运维任务已完成，Robotaxi 已激活下一排队任务" : "运维任务已完成，Robotaxi 恢复可运营" : `${getDisplayValue(result.task.task_status)}已完成`
  ...
  function assignWorker(taskId) {
    ...
    if (!task) return;
    const result = runManualBusinessAction(params => businessActionService.assignReadinessTask({
      ...params,
      objectId: taskId
    }));
  ...
  function startCheck(taskId) {
    const task = readinessTasks.find(item => item.task_id === taskId);
    if (!task) return;
    const result = runManualBusinessAction(params => businessActionService.startReadinessTask({
      ...params,
      objectId: taskId
    }));
  ...
  function submitCheckResult(taskId, checkResult, issueType = taskTypes.IssueType.NONE) {
    ...
    const result = runManualBusinessAction(params => passed ? businessActionService.passReadinessTask({
      ...params,
      objectId: taskId
    }) : businessActionService.failReadinessTask({
      ...params,
      objectId: taskId,
      issueType
    }));
  ...
  function createDeploymentTasks() {
    const result = runRepeatedManualBusinessAction(params => businessActionService.createDeploymentTask({
      ...params,
      runtime: {
        ...params.runtime,
        context: {
      ...
      maxIterations: data.robotaxis.length,
      isProgressResult: item => item.resultType === "DEPLOYMENT_CREATED"
    });
    const created = result?.data?.results || [];
    const triggerLog = createEventLog({
  ...
  function estimateServiceOrderPrice(serviceOrderId) {
    ...
    const result = runManualBusinessAction(params => businessActionService.executePricing({
      ...params,
  ...
  function callRobotaxiForServiceOrder(serviceOrderId) {
    ...
    const result = runManualBusinessAction(params => businessActionService.callRobotaxi({
      ...params,
      objectId: serviceOrderId
    }));
    if (!result?.success) return;
  ...
  function matchServiceOrder(serviceOrderId) {
    ...
    const result = runManualBusinessAction(params => businessActionService.executeOrderMatching({
      ...params,
      objectId: serviceOrderId
    }));
    const nextOrder = result?.updates?.serviceOrders?.find(order => order.service_order_id === serviceOrderId);
  ...
  function createTripForOrder(serviceOrderId) {
    ...
    const result = runManualBusinessAction(params => businessActionService.advanceTrip({
      ...params,
      objectId: serviceOrderId
    }));
  ...
  function settleServiceOrder(serviceOrderId, visibleOrder = null) {
    const serviceOrder = serviceOrders.find(item => item.service_order_id === serviceOrderId);
    const result = runManualBusinessAction(params => businessActionService.settleServiceOrder({
      ...params,
      objectId: serviceOrderId
    }));
    ...
    addServiceOrderEvent({
      ...
      message: result?.success ? `${serviceOrderId} 结算完成，最终价格 ${settledOrder?.final_price ?? visibleOrder?.final_price ?? serviceOrder?.final_price ?? ""}` : `${serviceOrderId} 结算失败：${getDisplayValue(result?.data?.failureReason)}`
    });
    selectForPage("serviceOrders", "serviceOrder", serviceOrderId);
  }
  function payServiceOrder(serviceOrderId) {
    ...
    const result = runManualBusinessAction(params => businessActionService.payServiceOrder({
      ...params,
      objectId: serviceOrderId
    }));
    if (!result?.success) return;
  ...
  function completeTripTravelNow(tripId) {
    ...
    const result = runManualBusinessAction(params => businessActionService.completeTripTravel({
      ...params,
      objectId: tripId
    }));
    if (!result?.success) return;
  ...
  function planRouteExecutionRoute(routeExecutionId) {
    ...
    addLog({
      ...
      message: serviceResult?.success ? `${task.task_id} 的行驶记录 ${routeExecutionId} 已通过服务层完成路径规划` : `${task.task_id} 路径规划失败：${getDisplayValue(serviceResult?.data?.failureReason)}`
    });
  ...
  function advanceRouteExecution(routeExecutionId) {
    ...
    const serviceResult = runManualBusinessAction(params => businessActionService.advanceRouteExecution({
      ...params,
      objectId: routeExecutionId
    }));
    ...
    if (serviceResult?.success && nextExecution) {
      addLog({
        event_type: nextExecution.execution_status === taskTypes.RouteExecutionStatus.ARRIVED ? taskTypes.TaskEventType.ROUTE_EXECUTION_ARRIVED : taskTypes.TaskEventType.ROUTE_STEP_ADVANCED,
        event_result: taskTypes.TaskEventResult.SUCCESS,
        task_id: nextExecution.task_id,
        robotaxi_id: nextExecution.robotaxi_id,
        route_execution_id: nextExecution.route_execution_id,
        message: nextExecution.execution_status === taskTypes.RouteExecutionStatus.ARRIVED ? `${nextExecution.robotaxi_id} 已到达当前路径目标，等待到达结果` : `${nextExecution.robotaxi_id} 继续行驶至 ${nextExecution.current_cell_id}`
      });
  ...
  function completeRouteExecutionTravelNow(routeExecutionId) {
    ...
    const serviceResult = runManualBusinessAction(params => businessActionService.completeRouteExecutionTravel({
      ...params,
      objectId: routeExecutionId
    }));
    const nextExecution = serviceResult?.updates?.routeExecutions?.find(item => item.route_execution_id === routeExecutionId);
  ...
  function submitNormalArrival(routeExecutionId) {
    ...
    const serviceResult = runManualBusinessAction(params => businessActionService.confirmRouteExecutionArrival({
      ...params,
      objectId: routeExecutionId
    }));
  ...
  function submitAbnormalArrival(routeExecutionId, arrivalResult) {
    ...
    addLog({
      ...
      robotaxi_id: execution.robotaxi_id,
      route_execution_id: execution.route_execution_id,
      message: serviceResult?.success ? `${execution.robotaxi_id} 异常到达：${getDisplayValue(arrivalResult)}` : serviceResult?.message || `${execution.robotaxi_id} 异常到达确认失败`
    });
...
function RecordTable({
  ...
  if (!config || !objectType) {
    return /*#__PURE__*/React.createElement("div", {
      ...
      description: "\u9875\u9762\u914D\u7F6E\u5DF2\u66F4\u65B0\uFF0C\u8BF7\u4ECE\u5DE6\u4FA7\u83DC\u5355\u91CD\u65B0\u6253\u5F00\u9875\u9762"
    }));
  }
  ...
  const robotaxiOperationActionMap = useMemo(() => {
    ...
    return new Map(displayRows.map(row => [row.robotaxi_id, actions.getRobotaxiFleetOperationActions(row)]));
  ...
  }, "\u751F\u6210\u6295\u653E\u4EFB\u52A1")), isFleetOperationTaskPage && /*#__PURE__*/React.createElement("div", {
    className: "list-action-bar"
  }, /*#__PURE__*/React.createElement(Button, {
    size: "small",
    type: "primary",
    onClick: () => actions.runFleetOperationPolicyForPage(page)
  }, getFleetOperationTaskActionLabel(page))), isServiceOrderPage && /*#__PURE__*/React.createElement("div", {
    className: "list-action-bar"
  }, /*#__PURE__*/React.createElement(Button, {
    size: "small",
    type: "primary",
  ...
  }, "\u6E05\u7A7A\u5168\u90E8\u4F5C\u4E1A")), isMetricAnalysisPage && /*#__PURE__*/React.createElement("div", {
    className: "list-action-bar metric-period-actions"
  }, /*#__PURE__*/React.createElement("span", null, "\u7EDF\u8BA1\u5468\u671F"), /*#__PURE__*/React.createElement(Select, {
    size: "small",
  ...
  }, isTripPage ? "履约行驶事件" : isServiceOrderPage ? "最近事件记录" : isSimulationRunPage ? "模拟运行事件" : isStrategyExecutionPanelPage ? "最近策略执行" : isDemandSimulationStrategyPage ? "需求模拟执行" : isRoutePlanningPage ? "路径规划执行记录" : isPricingPage ? "定价执行记录" : isOrderMatchingPage ? "匹配执行记录" : "最近任务事件"), /*#__PURE__*/React.createElement(Table, {
    size: "small",
    rowKey: eventRowKey,
  ...
  function getActionColumn() {
    ...
    if (isLongTermDemandForecastStrategyPage) {
      return {
        ...
        width: 120,
        render: (_, row) => renderActionCell(row, /*#__PURE__*/React.createElement(RowActionButton, {
          onClick: () => actions.runLongTermDemandForecastStrategy(row)
        }, "\u6267\u884C"))
      };
    }
    if (isLongTermDemandForecastPage) {
      return {
        ...
        width: 150,
        render: (_, row) => renderActionCell(row, /*#__PURE__*/React.createElement(RowActionButton, {
          onClick: () => actions.createSupplyPlanFromForecast(row)
        }, "\u751F\u6210\u751F\u4EA7\u8BA1\u5212"))
      };
    }
    if (isSupplyPlanPage) {
    ...
    if (isFleetAllocationStrategyPage) {
      return {
        ...
        width: 120,
        render: (_, row) => renderActionCell(row, /*#__PURE__*/React.createElement(RowActionButton, {
          onClick: () => actions.runFleetAllocationStrategy(row)
        }, "\u6267\u884C"))
      };
    }
    if (isFleetAllocationResultPage) {
      return {
        ...
        width: 150,
        render: (_, row) => renderActionCell(row, /*#__PURE__*/React.createElement(RowActionButton, {
          onClick: () => actions.createDeliveryOrderFromAllocationResult(row)
        }, "\u751F\u6210\u4EA4\u4ED8\u5355"))
      };
    }
    ...
    if (isRobotaxiPage) {
      return {
        ...
        fixed: "right",
        width: 320,
        render: (_, row) => renderActionCell(row, renderRobotaxiFleetOperationActions(row, {
          ...actions,
          page,
          objectType,
          idField,
          robotaxiOperationActionMap
        }))
      };
    }
    ...
    if (isFleetOperationPolicyPage) {
      return {
        ...
        fixed: "right",
        width: 150,
        render: (_, row) => renderActionCell(row, renderFleetOperationPolicyActions(row, {
          ...actions,
          page,
          objectType,
          idField
...
function createFleetOperationPolicyRunRows(policyRuns = [], policies = []) {
  ...
  return (policyRuns || []).filter(run => run.fleet_operation_policy_id && (visiblePolicyIds.size === 0 || visiblePolicyIds.has(run.fleet_operation_policy_id))).sort((left, right) => String(right.started_at || right.created_at || "").localeCompare(String(left.started_at || left.created_at || "")));
}
function createStrategyRunRows(runs = [], strategies = [], strategyIdField) {
  ...
  return (runs || []).filter(run => run[strategyIdField] && (visibleStrategyIds.size === 0 || visibleStrategyIds.has(run[strategyIdField]))).sort((left, right) => String(right.started_at || right.created_at || "").localeCompare(String(left.started_at || left.created_at || "")));
}
...
function getDetailTabs(selectedType, selectedObject) {
  if (selectedType === "robotaxi") {
    return [{
      key: "basic",
      label: "基础信息",
      keys: ["robotaxi_id", "fleet_id", "model_name", "automation_level", "availability_status", "operation_blocking_reason", "motion_status"]
    }, {
      key: "asset",
      label: "资产事实",
      keys: ["max_range_km", "battery_capacity_kwh", "current_battery_kwh", "battery_percent", "estimated_range_km", "lifetime_distance_km", "lifetime_battery_consumed_kwh", "lifetime_charged_energy_kwh", "completed_service_order_count", "completed_cleaning_count", "completed_charging_count", "completed_maintenance_count"]
    }, {
      key: "task",
      label: "任务状态",
  ...
  if (selectedType === "routeExecution") {
    return [{
      ...
      key: "progress",
      label: "行驶进度",
      keys: ["current_step_index", "total_step_count", "total_distance_km", "distance_traveled_km", "distance_remaining_km", "battery_consumed_kwh", "time_elapsed"]
    }, {
      key: "route",
      label: "路径信息",
      keys: ["route_id", "route_strategy_id", "current_target_service_area_id", "route_summary", "route_links_detail"]
    }, {
      key: "time",
      label: "时间与来源",
      keys: ["created_at", "simulation_created_at", "record_source", "simulation_run_id", "simulation_global_tick", "started_at", "completed_at", "simulation_completed_at", "failure_reason"]
  ...
  if (selectedType === "trip") {
    return [{
      ...
      key: "progress",
      label: "行驶进度",
      keys: ["current_step_index", "total_step_count", "total_distance_km", "distance_traveled_km", "distance_remaining_km", "battery_consumed_kwh", "time_elapsed", "arrival_execution_result", "exception_type"]
    }, {
      key: "route",
      label: "路径信息",
      keys: ["route_id", "route_planning_run_id", "route_summary", "route_links_detail"]
    }, {
      key: "time",
      label: "时间与来源",
  ...
  if (selectedType === "metricDefinition") {
    return [{
      key: "basic",
      label: "指标定义",
      keys: ["metric_definition_id", "metric_name_cn", "metric_name_en", "metric_layer", "metric_domain", "metric_status", "definition_version"]
    }, {
      key: "formula",
      label: "计算口径",
      keys: ["business_definition", "calculation_formula", "source_objects", "source_fields", "time_basis", "default_time_window", "supported_dimensions"]
    }, {
      key: "quality",
      label: "质量规则",
      keys: ["zero_denominator_rule", "data_readiness", "display_unit", "higher_is_better"]
    }];
  }
  if (selectedType === "metricCalculationRun") {
    return [{
      key: "basic",
      label: "计算信息",
      keys: ["metric_calculation_run_id", "metric_scope_type", "metric_period_type", "metric_period_label", "simulation_run_ids", "simulation_timeline_id", "calculation_status", "calculation_progress_percent", "started_at", "completed_at"]
    }, {
      key: "result",
      label: "计算结果",
      keys: ["metric_definition_count", "generated_metric_observation_count", "error_count", "calculation_errors", "algorithm_version"]
    }];
  }
  if (selectedType === "metricObservation") {
    return [{
      key: "basic",
      label: "指标结果",
      keys: ["metric_observation_id", "metric_definition_id", "metric_scope_type", "metric_period_type", "metric_period_label", "metric_value", "metric_unit", "quality_status", "quality_reason"]
    }, {
      key: "window",
      label: "窗口与维度",
      keys: ["simulation_run_ids", "simulation_timeline_id", "window_type", "window_start_seconds", "window_end_seconds", "window_label", "dimension_type", "dimension_id"]
    }, {
      key: "source",
      label: "来源解释",
      keys: ["numerator_value", "denominator_value", "source_record_count", "source_object_refs", "created_at"]
    }];
  }
  return [];
}
...
function renderCellValue(key, row) {
  ...
  if (key === "pending_task_queue" && Array.isArray(row[key])) {
    ...
    return /*#__PURE__*/React.createElement(Popover, {
      title: "\u5F85\u6267\u884C\u4EFB\u52A1\u961F\u5217",
      content: /*#__PURE__*/React.createElement(RobotaxiQueuePopover, {
        queueItems: row[key]
      }),
      placement: "bottomLeft",
      trigger: "hover"
    }, /*#__PURE__*/React.createElement(Text, {
      className: "table-queue-summary"
    }, summary));
  }
  if (key === "result") {
...
function MetricExperiencePanel({
  ...
  const insightSourceRows = allRows.filter(row => row.metric_scope_type === "OPERATING_PERIOD" && row.metric_period_type === metricPeriodType);
  ...
  const activeTrendDay = selectedTrendDay && trendDayOptions.some(option => option.value === selectedTrendDay) ? selectedTrendDay : trendDayOptions[0]?.value || null;
  const dailyDemandRows = createDailyDemandTrendRows(hourlyDemandRows);
  const displayedDemandRows = demandTrendMode === "DAILY" ? dailyDemandRows : hourlyDemandRows.filter(row => getMetricTrendDayValue(row) === activeTrendDay);
  ...
  const insightGroups = [createMetricInsightGroup({
    title: "财务结果",
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    onClick: () => group.primary?.row && onSelect(group.primary.row)
  }, /*#__PURE__*/React.createElement("span", null, group.title), /*#__PURE__*/React.createElement("strong", null, group.primaryText), /*#__PURE__*/React.createElement("small", null, group.secondaryText), /*#__PURE__*/React.createElement("em", null, group.description)))), page === "serviceMetrics" && /*#__PURE__*/React.createElement("div", {
    className: "metric-trend-toolbar"
  }, /*#__PURE__*/React.createElement("span", null, "\u9700\u6C42\u8D8B\u52BF"), /*#__PURE__*/React.createElement(Button, {
    size: "small",
    type: demandTrendMode === "HOURLY" ? "primary" : "default",
    onClick: () => setDemandTrendMode("HOURLY")
  }, "\u6309\u5C0F\u65F6"), /*#__PURE__*/React.createElement(Button, {
    size: "small",
    type: demandTrendMode === "DAILY" ? "primary" : "default",
    onClick: () => setDemandTrendMode("DAILY")
  }, "\u6309\u5929"), demandTrendMode === "HOURLY" && /*#__PURE__*/React.createElement(Select, {
    size: "small",
  ...
  })), page === "serviceMetrics" && /*#__PURE__*/React.createElement("div", {
    className: "metric-trend-grid"
  }, /*#__PURE__*/React.createElement(MetricTrendChart, {
  ...
  }, /*#__PURE__*/React.createElement("span", null, "\u6570\u636E\u6C60\uFF1A", calculationSummary), /*#__PURE__*/React.createElement("span", null, "\u5F53\u524D\u7EDF\u8BA1\u5468\u671F\uFF1A", periodLabel), /*#__PURE__*/React.createElement("span", null, "\u6307\u6807\u7ED3\u679C ", latestRows.length), /*#__PURE__*/React.createElement("span", null, "\u6765\u6E90\u8BB0\u5F55 ", sourceRecordCount), /*#__PURE__*/React.createElement("span", null, qualitySummary), /*#__PURE__*/React.createElement("span", null, warningRows[0] ? `${getMetricDisplayName(warningRows[0])}：${warningRows[0].quality_reason || "存在质量提示"}` : "暂无质量提示")));
}
function MetricTrendChart({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "metric-trend-title"
  }, /*#__PURE__*/React.createElement("span", null, title), /*#__PURE__*/React.createElement("small", null, rows.length ? `${rows.length} 个时间点` : "等待数据")), rows.length ? /*#__PURE__*/React.createElement("div", {
    className: "metric-trend-bars"
  }, rows.map(row => {
    ...
    return /*#__PURE__*/React.createElement("button", {
      ...
      className: "metric-trend-bar-track"
    }, /*#__PURE__*/React.createElement("span", {
      className: "metric-trend-bar-fill",
      style: {
        height
      }
    })), /*#__PURE__*/React.createElement("strong", null, formatMetricDisplayValue(row)), /*#__PURE__*/React.createElement("small", null, formatTrendDimensionLabel(row)));
  })) : /*#__PURE__*/React.createElement("div", {
    className: "metric-trend-empty"
  }, emptyText));
}
function createMetricTrendRows(rows = [], metricDefinitionId) {
  return (rows || []).filter(row => row.metric_definition_id === metricDefinitionId).sort((left, right) => Number(left.window_start_seconds || 0) - Number(right.window_start_seconds || 0));
}
...
function RobotaxiOperationPanel({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    label: "\u5F53\u524D\u4EFB\u52A1",
    value: formatRobotaxiCurrentTask(activeRow)
  }), /*#__PURE__*/React.createElement(RobotaxiSummaryItem, {
    label: "\u5F85\u5904\u7406",
    value: formatRobotaxiOperationNeedSummary(activeRow)
  }), /*#__PURE__*/React.createElement(RobotaxiSummaryItem, {
    label: "\u5F85\u6267\u884C\u961F\u5217",
...
function renderSupplyPlanActions(row, actions) {
  if (row.plan_status === "DRAFT") {
    return /*#__PURE__*/React.createElement(RowActionButton, {
      onClick: () => actions.confirmSupplyPlan(row)
    }, "\u786E\u8BA4\u8BA1\u5212");
  }
  if (row.plan_status === "CONFIRMED") {
    return /*#__PURE__*/React.createElement(RowActionButton, {
      onClick: () => actions.createProductionBatchFromSupplyPlan(row)
    }, "\u751F\u6210\u751F\u4EA7\u6279\u6B21");
  }
...
function renderProductionBatchActions(row, actions) {
  if (row.batch_status === "PLANNED") {
    return /*#__PURE__*/React.createElement(RowActionButton, {
      onClick: () => actions.startProductionBatch(row)
    }, "\u5F00\u59CB\u751F\u4EA7");
  }
  if (row.batch_status === "IN_PRODUCTION") {
    return /*#__PURE__*/React.createElement(RowActionButton, {
      onClick: () => actions.completeProductionBatch(row)
    }, "\u751F\u4EA7\u5B8C\u6210");
  }
  return renderViewDetailAction(row, actions);
}
function renderRobotaxiDeliveryOrderActions(row, actions) {
  if (row.delivery_status === "CREATED") {
    return /*#__PURE__*/React.createElement(RowActionButton, {
      onClick: () => actions.startDeliveryOrder(row)
    }, "\u5F00\u59CB\u4EA4\u4ED8");
  }
  if (row.delivery_status === "IN_DELIVERY") {
    return /*#__PURE__*/React.createElement(RowActionButton, {
      onClick: () => actions.completeDeliveryOrder(row)
    }, "\u4EA4\u4ED8\u5B8C\u6210");
  }
  return renderViewDetailAction(row, actions);
}
...
function renderFleetOperationTaskActions(row, actions) {
  ...
  if (row.task_type === "RETIREMENT" && status === "WAITING_RETIREMENT_APPROVAL") {
    return /*#__PURE__*/React.createElement(RowActionGroup, null, /*#__PURE__*/React.createElement(RowActionButton, {
      onClick: () => actions.approveRetirementTask(row)
    }, "\u786E\u8BA4\u9000\u5F79"), /*#__PURE__*/React.createElement(RowActionButton, {
      type: "default",
      onClick: () => actions.rejectRetirementTask(row)
    }, "\u9A73\u56DE\u9000\u5F79"));
  }
  ...
  if (isFleetOperationWorkActiveStatus(row)) {
    return /*#__PURE__*/React.createElement(RowActionButton, {
      onClick: () => actions.completeFleetOperationWork(row)
    }, getFleetOperationCompleteWorkLabel(row));
  }
  return /*#__PURE__*/React.createElement(RowActionButton, {
    type: "default",
    onClick: () => actions.viewRecordDetail(actions.page, actions.objectType, row[actions.idField])
...
function isFleetOperationRouteRecordStatus(row) {
  ...
  return ["WAITING_ROUTE", "MOVING_TO_OPS_CENTER", "MOVING_TO_CHARGER", "MOVING_TO_MAINTENANCE_CENTER", "MOVING_TO_RETIREMENT_CENTER", "ARRIVED_OPS_CENTER", "ARRIVED_CHARGER", "ARRIVED_MAINTENANCE_CENTER", "ARRIVED_RETIREMENT_CENTER", "ARRIVAL_ABNORMAL"].includes(row.task_status);
}
...
function isFleetOperationReadyToStartStatus(row) {
  return ["READY_TO_CLEAN", "READY_TO_START", "READY_TO_REPAIR", "READY_TO_CHARGE"].includes(row?.task_status);
}
function isFleetOperationWorkActiveStatus(row) {
  return ["CLEANING", "CLEANING_IN_PROGRESS", "REPAIRING", "MAINTENANCE_IN_PROGRESS", "CHARGING", "READY_TO_DISCONNECT", "DIAGNOSING", "PROCESSING_RETIREMENT"].includes(row?.task_status);
}
...
function renderOperationTags(row) {
  const tags = [];
  if (row.needs_cleaning) tags.push(React.createElement("span", {
    key: "cln",
  ...
  if (row.needs_charging) tags.push(React.createElement("span", {
    key: "chg",
  ...
  if (row.needs_maintenance) tags.push(React.createElement("span", {
    key: "mnt",
  ...
  if (row.failure_status && row.failure_status !== "NONE") tags.push(React.createElement("span", {
    key: "flr",
    className: "tag tag-danger",
    style: {
      marginRight: 4
  ...
  return tags.length > 0 ? React.createElement("span", null, ...tags) : React.createElement("span", {
    style: {
      color: "var(--muted)"
    }
  }, "无");
...
function renderRobotaxiFleetOperationActions(row, actions) {
  ...
  const availableActions = actions.robotaxiOperationActionMap?.get(row.robotaxi_id) || (actions.getRobotaxiFleetOperationActions ? actions.getRobotaxiFleetOperationActions(row) : []);
  ...
  return /*#__PURE__*/React.createElement(RowActionGroup, null, can(taskTypes.TaskType.CLEANING) && /*#__PURE__*/React.createElement(RowActionButton, {
    onClick: () => actions.createDirectFleetOperationTaskFromRobotaxi(row, taskTypes.TaskType.CLEANING)
  }, "\u6E05\u6D01"), can(taskTypes.TaskType.CHARGING) && /*#__PURE__*/React.createElement(RowActionButton, {
    type: "default",
    onClick: () => actions.createDirectFleetOperationTaskFromRobotaxi(row, taskTypes.TaskType.CHARGING)
  }, "\u5145\u7535"), can(taskTypes.TaskType.MAINTENANCE) && /*#__PURE__*/React.createElement(RowActionButton, {
    type: "default",
    onClick: () => actions.createDirectFleetOperationTaskFromRobotaxi(row, taskTypes.TaskType.MAINTENANCE)
  }, "\u7EF4\u4FEE"), can(taskTypes.TaskType.FAILURE_HANDLING) && /*#__PURE__*/React.createElement(RowActionButton, {
    type: "default",
    danger: true,
    onClick: () => actions.createDirectFleetOperationTaskFromRobotaxi(row, taskTypes.TaskType.FAILURE_HANDLING)
  }, "\u6545\u969C"), availableActions.length === 0 && /*#__PURE__*/React.createElement(RowActionButton, {
    type: "default",
    onClick: () => actions.viewRecordDetail(actions.page, actions.objectType, row[actions.idField])
  }, "\u67E5\u770B\u8BE6\u60C5"));
}
function renderFleetOperationPolicyActions(row, actions) {
  return /*#__PURE__*/React.createElement(RowActionGroup, null, /*#__PURE__*/React.createElement(RowActionButton, {
    type: "default",
    onClick: () => actions.editFleetOperationPolicy(row)
  }, "\u914D\u7F6E"), /*#__PURE__*/React.createElement(RowActionButton, {
    onClick: () => actions.runFleetOperationPolicy(row.target_task_type)
  }, "\u6267\u884C"));
}
...
function renderTripActions(row, actions) {
  ...
  if (row.trip_status === "ON_THE_WAY_PICKUP") {
    return /*#__PURE__*/React.createElement(RowActionGroup, null, /*#__PURE__*/React.createElement(RowActionButton, {
      ...
      type: "default",
      onClick: () => actions.completeTripTravelNow(row.trip_id)
    }, "\u81EA\u52A8\u5230\u8FBE"), /*#__PURE__*/React.createElement(RowActionButton, {
      type: "default",
      danger: true,
...
function isCandidateRobotaxi(robotaxi, tasks) {
  return ["PENDING_ADMISSION", "PENDING_INSPECTION"].includes(robotaxi.availability_status) && !tasks.some(task => task.robotaxi_id === robotaxi.robotaxi_id && unfinishedTaskStatuses.has(task.task_status));
}
function isDeploymentCandidateRobotaxi(robotaxi, context = {}) {
  ...
  const hasOpenDeployment = deploymentTasks.some(task => task.robotaxi_id === robotaxi.robotaxi_id && unfinishedDeploymentStatuses.has(task.task_status));
...
function attachCostRecords(item, objectType, costRecords = [], routeExecutions = []) {
  ...
  if (objectType === "serviceOrder") {
    ...
    records = costRecords.filter(record => record.source_object_type === objectType && record.source_object_id === item.task_id || item.route_execution_id && record.source_object_type === "routeExecution" && record.source_object_id === item.route_execution_id);
  }
  return {
    ...item,
...
function updateRobotaxiForTrip(robotaxi, trip, previousTrip = {}) {
  ...
  const batteryDeltaPercent = robotaxi.battery_capacity_kwh ? Number((nextBatteryDeltaKwh / robotaxi.battery_capacity_kwh * 100).toFixed(2)) : robotaxi.max_range_km ? Number((distanceDeltaKm / robotaxi.max_range_km * 100).toFixed(2)) : Number(Math.max(0, Number(trip.battery_consumed_percent || 0) - Number(previousTrip.battery_consumed_percent || 0)).toFixed(2));
...
function createMenuItems(items = []) {
  return items.filter(item => !hiddenWorkspacePages.has(item.key)).map(item => {
...
function loadRuntimeSnapshot(initialData) {
  ...
  try {
    ...
    const taskDispatchStrategies = Array.isArray(snapshot.taskDispatchStrategies) && snapshot.taskDispatchStrategies.length ? snapshot.taskDispatchStrategies : taskDispatchStrategyService.initializeDefaultTaskDispatchStrategies();
    const robotaxiTaskPlanningStrategies = Array.isArray(snapshot.robotaxiTaskPlanningStrategies) && snapshot.robotaxiTaskPlanningStrategies.length ? snapshot.robotaxiTaskPlanningStrategies : robotaxiTaskPlanningService.initializeDefaultRobotaxiTaskPlanningStrategies();
    ...
    const taskPriorityConfigs = Array.isArray(snapshot.taskPriorityConfigs) && snapshot.taskPriorityConfigs.length ? snapshot.taskPriorityConfigs : [robotaxiTaskPriorityService?.initializeDefaultPriorityConfig() || {}];
    ...
    const metricDefinitions = Array.isArray(snapshot.metricDefinitions) && snapshot.metricDefinitions.length ? metricCalculator.normalizeMetricDefinitions(snapshot.metricDefinitions) : metricCalculator.initializeDefaultMetricDefinitions();
...
function createMetricInsightGroup({
  title,
  description,
  primary,
  secondary = []
  ...
  return {
    ...
    secondaryText: secondaryPresentations.length ? secondaryPresentations.map(item => item.text).join(" / ") : "等待周期计算结果"
  };
}
