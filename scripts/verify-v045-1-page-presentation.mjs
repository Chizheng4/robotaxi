...
assert.match(stylesSource, /\.detail-tabs \.ant-tabs-nav-wrap\s*\{[^}]*overflow-x:\s*auto/s, "详情页签必须使用浏览器原生横向滚动");
assert.match(stylesSource, /\.detail-tabs \.ant-tabs-nav-wrap\s*\{[^}]*scrollbar-width:\s*none/s, "详情页签不得显示灰色滚动条");
...
assert.match(tabbedDetailSource, /detail-tabs-native-scroll[^>]*onWheelCapture=\{\(event\) => event\.stopPropagation\(\)\}/, "详情页签必须隔离组件对原生滚轮的二次接管");
assert.match(stylesSource, /\.detail-tabs \.ant-tabs-nav-list\s*\{[^}]*transform:\s*none/s, "详情页签原生滚动时不得叠加组件内部位移");
