...
assert(styles.includes("position: fixed;\n  top: var(--app-viewport-offset-top)"), "登录画布必须脱离根页面并跟随真实可视视口");
assert(styles.includes("top: clamp(72px, 12svh, 108px)"), "手机登录表单必须锚定在首屏上半部安全区");
assert(styles.includes("left: var(--app-viewport-offset-left)"), "登录画布必须跟随可视视口水平偏移");
assert(styles.includes("width: var(--app-viewport-width)"), "登录画布必须使用真实可视视口宽度");
assert(styles.includes("right: max(20px, env(safe-area-inset-right))"), "手机登录表单必须保留稳定左右安全边距");
assert(styles.includes("-webkit-text-size-adjust: 100%"), "WebView 必须关闭自动文字膨胀");
...
cycle.focusInput();
cycle.resizeViewport(490, 28, 18);
assert.equal(cycle.root.dataset.keyboardOpen, "true");
assert.equal(cycle.root.style.values.get("--app-viewport-offset-left"), "18px", "WebView 水平偏移必须投影到可视画布");
...
function createViewportCycleHarness() {
  ...
  return {
    ...
    resizeViewport(height, offsetTop, offsetLeft = 0) {
      visualViewport.height = height;
      visualViewport.offsetTop = offsetTop;
      visualViewport.offsetLeft = offsetLeft;
