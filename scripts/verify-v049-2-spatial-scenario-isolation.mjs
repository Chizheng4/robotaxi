...
assert.equal(baseScene.zones.features.length, 2);
...
const isolatedScene = createCityGeographicScene({ catalog: CITY_SPATIAL_CATALOG, plans: [foreignPlan] }, GEOSPATIAL_MAP_DATASET);
assert.equal(isolatedScene.zones.features[0].properties.spatial_plan_id, undefined);
const publishedScene = createCityGeographicScene({ catalog: CITY_SPATIAL_CATALOG, plans: [published] }, GEOSPATIAL_MAP_DATASET);
assert.equal(publishedScene.zones.features[0].properties.spatial_plan_id, published.operating_spatial_plan_id);

const started = performance.now();
for (let index = 0; index < 500; index += 1) {
  createCityGeographicScene({ catalog: CITY_SPATIAL_CATALOG, plans: [published] }, GEOSPATIAL_MAP_DATASET);
}
