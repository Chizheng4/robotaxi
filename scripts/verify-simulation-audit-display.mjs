...
assert.match(source, /function getStructuredKeyLabel\(key\)/, "复杂详情字段必须通过统一结构化字段显示服务处理");
assert.match(source, /<dt>\{getStructuredKeyLabel\(key\)\}<\/dt>/, "复杂详情字段必须通过统一字段字典或枚举字典显示中文");
