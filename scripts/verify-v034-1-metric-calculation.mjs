...
assert.equal(observationByMetricId.get("OUTCOME-EFF-003").metric_value, 15, "单均贡献利润必须等于贡献利润除以完成订单数");
...
assert.equal(observationByMetricId.has("PROCESS-SUPPLY-001"), false, "不同供给单据不得混合为一个无明确分母的完成率");
...
assert.equal(observationByMetricId.get("DEMAND-TREND-001").metric_value, 2, "每小时订单数必须来自服务订单创建时间");
assert.equal(result.metricObservations.find((item) => item.metric_definition_id === "DEMAND-TREND-002" && item.dimension_id === "OFF_PEAK").metric_value, 2, "低峰时段订单数必须来自服务订单创建时间");
assert.equal(result.metricObservations.find((item) => item.metric_definition_id === "DEMAND-TREND-002" && item.dimension_id === "PEAK").metric_value, 0, "时段订单数必须保留高峰空桶，便于前端趋势展示");
...
assert.match(mainSource, /财务表现/, "经营分析必须接入财务表现");
assert.match(mainSource, /服务分析/, "经营分析必须接入服务分析");
assert.match(mainSource, /过程诊断/, "经营分析必须接入过程诊断");
assert.match(mainSource, /财务管理/, "收入、成本和财务计算记录必须归入财务管理一级菜单");
assert.match(mainSource, /数据计算/, "指标定义、观测和计算记录必须归入数据计算分组");
...
assert.match(mainSource, /!isMetricAnalysisPage && !isForecastAnalysisPage && statusOptions\.length > 0/, "经营分析页不得显示业务对象状态分类");
assert.match(mainSource, /!isMetricAnalysisPage && !isForecastAnalysisPage && \(/, "经营分析页不得显示业务对象查询筛选栏");
