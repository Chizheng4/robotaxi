...
assert.match(mainSource, /服务经营分析/, "经营分析管理必须接入服务经营分析");
assert.match(mainSource, /运营过程诊断/, "经营分析管理必须接入运营过程诊断");
assert.match(mainSource, /财务经营管理/, "收入、成本和财务计算记录必须归入财务经营管理一级菜单");
assert.match(mainSource, /数据计算管理/, "指标定义、观测和计算记录必须归入数据计算管理分组");
