...
assert.match(navigationSource, /财务表现/, "经营分析必须接入财务表现");
assert.match(navigationSource, /服务分析/, "经营分析必须接入服务分析");
assert.match(navigationSource, /过程诊断/, "经营分析必须接入过程诊断");
...
assert.match(mainSource, /!isMetricAnalysisPage && !isForecastAnalysisPage && !isOperatingModelPage && statusOptions\.length > 0/, "经营分析页不得显示业务对象状态分类");
assert.match(mainSource, /!isMetricAnalysisPage && !isForecastAnalysisPage && !isOperatingModelPage && \(/, "经营分析页不得显示业务对象查询筛选栏");
