...
const calculationModels = fs.readFileSync(new URL("../src/domain/calculationModelRegistry.js", import.meta.url), "utf8");
...
assert(main.includes('dataChartService.js?v=20260717-v047-1-0'), "图表服务动态模块必须使用当前版本缓存标识");
assert(/businessPlanningService\.js\?v=20260719-v047-4-1/.test(main), "经营规划服务动态模块必须使用当前计算合同缓存标识");
...
assert(planning.includes("createRegisteredCalculationStep"), "预测计算步骤必须统一调用计算模型注册服务");
assert(calculationModels.includes("output_field: model.output_field"), "计算步骤必须声明唯一输出字段");
assert(calculationModels.includes("formula_expression: model.formula"), "计算步骤必须保存字段键公式");
assert(calculationModels.includes("source_refs: sourceRefs"), "计算步骤必须保存来源引用");
