...
`growth_adjustment_rate` 是预测策略的情景修正，不是地点或区域画像增长率。默认值为每月 0.50%。地点配置保存后重算 Place、ServiceArea 与 Zone 当前画像；历史预测结果保持不可变，新执行才读取新的区域增长率。前端统一显示为“策略增长调整率”，避免误解为画像汇总结果。
...
- 比例字段在配置界面以百分比输入和展示，领域对象统一保存为 `[0, 1]`；增长率大于 `-1`；
