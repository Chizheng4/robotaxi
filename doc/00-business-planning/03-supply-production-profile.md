...
```text
预测期可生产数量
= 预测期内各生产周期的生产能力 × 对应爬坡比例之和
```

```text
预测期可供应数量
= 预测期内同时完成生产、质量检验并满足交付能力约束的数量

计划当期生产量
= min(剩余计划生产数量, 每期生产能力 × 当前爬坡比例)
```
...
建议生产数量表达需求预测得到的 Robotaxi 缺口；预测期可生产数量和预测期可供应数量表达生产画像的能力上限，不受建议生产数量截断。供应决策才根据需补充供应数量、可生产数量和可交付数量确定计划 Robotaxi 数，三类字段不得混用。
...
生产画像必须关联生产工厂，描述该工厂版本化的生产、质检与交付能力。生产画像不直接修改生产计划、生产批次、Robotaxi、交付单或运营准入任务；生产计划确认时只冻结画像快照，后续画像变更不能回改已下达批次。
