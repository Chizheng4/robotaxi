...
现有模拟网格不是过渡页面，必须长期保留并可随时切换：

- 地理模式用于真实底图浏览和运营空间建模；
- 网格模式用于确定性模拟、历史功能回归和高速运行；
- 切换模式只改变空间投影，不重置业务数据、任务、订单、Robotaxi 或模拟运行；
...
| 模式 | 定位 | 主要用途 |
| --- | --- | --- |
| 模拟网格模式 | 基于 Cell 的确定性离散空间 | 高速模拟、回归测试、固定演示数据 |
| 真实地理模式 | 基于经纬度、GeoJSON 和真实道路网络 | 真实空间展示、区域编辑、道路路径规划 |

两种模式共享业务对象和业务服务，不共享页面私有数据。

```mermaid
flowchart LR
  OBJECT["统一空间业务对象"] --> SPATIAL["空间对象服务"]
  SPATIAL --> GRID["模拟网格投影"]
  SPATIAL --> GEO["真实地理投影"]
  GRID --> GRIDMAP["现有地图场景"]
  GEO --> MAP["真实地图适配器"]
...
地图绘制不能直接修改 Zone、Place 或 ServiceArea。系统新增独立的 `OperatingSpatialPlan`（运营空间方案），承接一次地图建模工作的草案、校验、预览和发布：
...
- 已有关联对象编号时，发布几何覆盖该对象在地理模式中的 Cell 投影几何；
- 新要素可以作为待初始化空间对象显示，但在完成 Cell、道路、画像和运营归属初始化前，不进入模拟运行和正式单据匹配；
...
初期只允许编辑项目自有业务几何，不提供修改 OSM 底图的能力。
...
- 单指移动、双指缩放，不依赖悬停；
- 轻触显示摘要，再按需展开底部或侧边详情；
- 编辑能力初期可以限制为查看和简单点位标记，复杂多边形编辑优先桌面端；
