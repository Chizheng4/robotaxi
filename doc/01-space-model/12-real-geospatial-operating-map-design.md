...
| 能力 | 推荐组件 | 项目责任边界 |
| --- | --- | --- |
| 地图渲染 | MapLibre GL JS | 城市地理浏览与规划的唯一渲染、图层、视口和交互引擎 |
...
禁止把公共 OSM 标准瓦片服务作为不可替换的正式依赖。底图来源必须可配置；外部底图失败时由 MapLibre 切换本地空样式并继续呈现平台运营图层，不切换第二地图引擎。浏览器不支持 WebGL、MapLibre 未加载或引擎创建失败时，城市地图必须明确标记不可用并关闭规划入口，同时保留业务对象列表和网格仿真切换，不得用静态图或不完整二维地图冒充正式规划能力。
...
运营区域方案是一次规划事务，不是正式空间对象。它只有 `草稿 / 已发布 / 已取消` 三类生命周期状态；校验通过或失败是独立的服务结果，不能成为方案生命周期状态。选择物理空间单元后自动保存草稿并自动校验，用户的主要命令只有“发布”；发布事务必须再次校验并一次性完成正式对象、关系、几何版本和事件的写入。

正式 Zone、Place 和 ServiceArea 独立维护 `有效 / 已停用` 生命周期，以及 `当前版本 / 已被替代` 版本状态：

- 已发布对象停用后退出当前经营空间，但历史几何、关系、来源和版本继续可追溯；
- 已停用对象可通过独立启用动作恢复；
- 编辑已发布对象时，当前版本持续有效，直到新草稿发布成功后才被新版本替代；
- 发布失败不得部分更新对象、关系或目录版本。
...
城市地理正式目录初始为空。Zone、Place、ServiceArea 必须由规划人员基于广州真实地图的物理空间单元选择、自动校验并发布后形成；页面、初始化数据和网格仿真都不得预置不可调整的城市空间对象。用户绘制的范围只是查询和粗选手势，不是正式对象边界。正式边界由空间服务根据选中的行政区、建筑、土地利用、地点、道路接入或上下车空间等来源单元派生。后续若引入导入数据，也必须通过同一方案发布和目录版本机制进入，不得成为页面私有标记。
...
选择空间对象、对象类型与名称
  ...
  → 用户圈定查询范围，系统选出范围内可用的物理空间单元
  → 地图突出显示已选单元，用户可排除误选单元
  → 空间服务按来源单元派生正式几何、父级关系和服务接入关系
  → 系统自动保存草稿并自动校验几何、层级、来源版本和可服务性
  → 用户发布；发布事务再次校验并生成正式对象与不可变目录版本
```

底图物理空间单元是城市空间对象边界的来源事实，但不是新的 Zone、Place、ServiceArea 或 Road 业务事实。方案保存稳定来源编号、来源图层、数据集与版本、来源几何和必要名称快照；正式对象保存由这些来源单元派生的标准 GeoJSON、中心点、包围盒、面积、层级、道路接入锚点和来源追溯。地点优先选择建筑、地块、POI 或土地利用面；服务区域必须关联地点，并选择可供上下车、临停、停车或待命的道路接入与场地单元。只覆盖水域或没有可用物理单元时禁止发布。

底图要素采集必须遵守稳定性合同：

- 圈选完成是显式服务调用，不依赖伪造键盘事件或浏览器偶然触发的绘制事件；
- 地图、样式或瓦片尚未加载完成时必须阻止选择，并说明等待原因；
- 采集同时读取当前地图数据源与可见图层，不得仅依赖某个缩放级别下已经渲染的标签；
- 没有识别到符合对象类型的物理空间单元时不得保存空草稿；
- 同一来源单元集合和同一地图数据版本重复执行，必须派生同一正式几何和校验结论；
- 页面不得把用户圈选多边形写入正式对象几何，也不得绕过来源单元直接发布。
...
# v049.12 城市地理对象工作台与导航边界

## 导航与对象边界

地图空间采用配置化的三级导航，页面归属与业务数据归属分离：

- 网格仿真：网格地图、网格单元、道路、道路节点、道路片段、地点、服务区域、运营区域。
- 城市地理：城市地图、运营区域、地点、服务区域。

网格仿真继续读取既有网格对象和生命周期，不迁移、不复制业务数据。城市地理读取由城市物理空间、运营区域规划草稿和正式发布结果共同形成的城市空间目录。相同业务概念可保持兼容字段，但坐标、几何和来源事实不能混用。

`规划运营区域` 是城市地图中的对象创建与调整入口，不是对象管理页面。发布后的城市运营区域、地点和服务区域必须进入独立对象工作台，具备标准列表、详情、地图定位、范围调整、停用、启用和版本追溯能力。

## 页面与服务合同

导航树只声明菜单层级、名称和页面键；页面架构注册表声明地图页或对象页；城市空间工作台服务把城市空间目录投影为统一对象记录。页面不得自行拼装城市对象。

城市对象状态以服务结果为准：

- 草稿只存在于规划服务中，可重做或撤销。
- 发布后形成正式对象并进入可用状态。
- 正式对象可以停用或重新启用；范围调整生成新草稿/新版本，不直接改写已发布几何事实。

城市地图和网格地图共用放大、缩小、复位交互规范。城市边界和正式运营区域使用低干扰半透明填充、双层边界和随缩放变化的信息层级，保证底图可读且对象范围可识别。

# v049.12.1 城市范围与正式运营区域视觉闭环

城市地理地图按“底图事实、城市范围、正式运营对象、规划草稿、交互状态”建立固定视觉层级：

- 广州城市范围在城市级视角使用由行政边界派生的外部轻遮罩、克制的半透明底衬和浅色外沿加深色内沿，用户无需寻找细线即可识别城市边界；进入区级视角后退出自有城市标签，避免与底图地名重复。
- 正式运营区域沿用规划草稿的清晰覆盖语言，但降低饱和度和强调强度；使用半透明填充和双层边界，保证发布前后范围识别能力连续。
- 规划草稿继续表达“正在选择”，正式区域表达“已经生效”，选中状态只强调当前对象，不改变业务对象颜色语义。
- 发布结果必须在发布按钮附近即时反馈；成功后按钮进入“已发布”不可重复操作，地图聚焦正式范围。错误反馈也在同一操作区显示，不依赖滚动到面板底部。
- 手机端不开放规划编辑，但必须能够清楚浏览城市范围和正式运营区域；业务图层不得遮蔽底图道路与地名。

验收必须同时覆盖城市远景、运营区域中景、对象选中状态、发布成功反馈和窄屏只读地图。只增加线宽或只在规划预览中显示清晰范围，不视为完成。

# v049.12.2 真实渲染视觉合同

v049.12.1 证明了空间图层和发布反馈代码存在，但没有证明这些图层在真实底图和实际视口中清晰可见。后续验收固定区分两类事实：

- 源码合同：图层、缩放区间、颜色令牌、标签过滤和图层顺序存在；
- 渲染事实：地图进入稳定状态后，城市范围或正式运营区域在当前视口实际产生可见要素。

城市范围从广州全域视角开始显示，城市底衬进入区级视角后逐步退出，边界继续短暂保留以形成连续过渡。正式一级运营区域从城市运营视角开始显示，二级区域只在更近视角出现。所有业务范围填充和边界位于底图文字下方，底图道路、建筑和有效地名仍然可读。

底图自带的广州城市标签必须精确过滤，由平台城市标签承担城市级范围说明；进入区级视角后平台城市标签也退出。真实浏览器验收必须读取地图稳定后的渲染诊断，并结合截图确认视觉层级，不能再以字符串或图层存在代替用户可见结果。

地图适配器向 MapLibre 安装图层时，未定义的 `minzoom`、`maxzoom` 和 `filter` 必须省略，不能以 `undefined` 进入图层配置；一个空间来源拆成填充、边界和外沿等嵌套图层时，子图层必须显式声明类型或继承来源类型。该合同同时由自动检查和真实浏览器渲染诊断约束。
