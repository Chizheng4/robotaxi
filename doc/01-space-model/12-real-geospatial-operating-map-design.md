...
广州城市地图在低缩放层级显示“广州受控演示范围”，用于说明当前地图数据集的可规划边界。它是地图数据集参考范围，不是经营意义上的 Zone，不参与画像、预测、供应、单据或模拟运行，也不可通过运营区域规划功能编辑。正式 Zone 只能由当前城市空间规划合同创建、校验和发布。城市参考范围使用轻量填色和边界线表达；正式空间对象的区域面与文字标签共享同一悬停、选中和详情交互，不能出现“看得到名称但选不中对象”。
