...
初始化对象与后续新增对象遵守同一治理能力：均可从地图选中后调整；不再使用的对象通过版本化方案执行软停用并退出当前有效场景，禁止页面硬删除。停用 ServiceArea 可独立完成；停用 Place 前必须处理其有效 ServiceArea 和运营中心关系；停用 Zone 前必须处理其有效子 Zone、Place、ServiceArea 和运营中心。历史几何、来源方案和版本继续保留。
...
规划对象类型由当前地图视角约束，而不是要求用户在表单中拼装层级关系：城市视野规划一级 Zone；区域视野在已有一级 Zone 内规划可选二级 Zone；地点视野规划 Place；道路与建筑视野规划 ServiceArea。视角变化时，新建对象类型同步变化；编辑已发布对象时保持原对象类型。
