...
“结果”是策略链中的业务角色，不代表所有结果必须使用同一种物理对象。决策能力必须声明结果性质：

|结果性质|含义|当前典型对象|
|---|---|---|
|分析结果|保存预测或分析结论，不承担履约生命周期|长期需求预测结果、短期需求预测结果|
|决策记录|保存一次可审计的选择或判断|定价决策、订单匹配决策、运维调度决策|
|计划单据|决策形成的正式经营承诺，具有确认、取消和下游执行关系|生产计划、投放计划|
|运营对象|可被业务执行直接使用的运营资源|Route|
|模拟结果|只属于模拟环境的分析输出|需求模拟结果|

当结果已经是具有独立编号、状态、动作、生命周期和下游执行关系的正式计划单据时，不再额外创建内容重复的“策略结果”对象。分析结果、决策记录、计划单据和运营对象必须保持各自边界，页面不能因为都处于“结果页”而使用相同业务含义。
...
策略用于解释模型、算法、规则、参数和适用条件；执行记录保存本次真实输入快照、模型版本、公式、步骤、中间结果、最终结果、来源和诊断；结果展示关键结论与形成依据，并引用同一次执行过程。正式计划单据只保存来源执行编号、关键决策摘要和追溯入口，不复制另一套计算事实。

统一计算步骤至少包含：模型编号、模型版本、输入字段和值、来源引用、公式、输出字段和值、单位和诊断信息。历史对象中的不同计算步骤字段允许兼容读取，但新增决策能力只能写入统一的 `calculation_steps` 合同。
...
效果评价不在决策中心重复建模。每项决策能力关联统一经营指标，例如订单匹配成功率、路径规划成功率、履约率、资产覆盖率和经营贡献；经营分析计算后供决策中心引用。
