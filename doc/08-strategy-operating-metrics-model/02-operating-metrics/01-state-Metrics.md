...
|待准入 Robotaxi|已交付但尚未完成运营准入的车辆数|Robotaxi、运营准入任务|Zone、运营中心|
|执行中服务订单|截止点尚未进入终态的服务订单数|服务订单|Zone、订单状态|
|执行中履约行驶|截止点尚未进入终态的履约行驶记录数|履约行驶记录|Zone、Robotaxi|

状态指标必须保存统计截止时间和时间口径。历史状态趋势只有在存在对应快照时才能计算，不得用当前状态回填历史。状态数量用于解释当前承载和积压，不进入期间成功率分母。
