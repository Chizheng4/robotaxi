...
- `Z-001`：测试运营区，继续承载现有 Robotaxi、订单、任务、路径和模拟运行；
- `Z-002`：规划区域，拥有完整道路、地点、服务区域和草稿画像，但尚不进入需求、匹配、路径、运维和模拟候选。

区域是否可以参与运营由 Zone 及其空间对象状态共同决定，不能因为对象已经显示在地图上就视为可运营。
...
12. 地图可以展示规划态 Zone，但业务服务只能消费满足运营状态合同的 Zone 及其对象。
