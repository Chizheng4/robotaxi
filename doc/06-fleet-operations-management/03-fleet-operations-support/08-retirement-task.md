...
等待中的 RetirementTask 不占用 Robotaxi.current_task_id。退役确认并接管后，Robotaxi 进入运维中并从可运营集合、投放集合和订单匹配集合中移除；只有退役处理完成后，Robotaxi 才进入已退役。
...
1. 已退役 Robotaxi 不得再参与订单匹配、投放、准入、充电、清洁和维修。
...
6. 退役驳回时，Robotaxi 应回到可运营或原可执行状态；退役完成才进入已退役。
