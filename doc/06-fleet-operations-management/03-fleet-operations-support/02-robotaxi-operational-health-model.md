...
它不是新的业务单据，也不是独立的标签对象，而是 Robotaxi 本体用于汇总综合状态、判断是否可运营、是否可触发任务、任务应立即执行还是排队的底层服务化模型。

```text
Robotaxi 运营健康模型 = Robotaxi 本体事实 + 当前任务 / 队列 + 可运营判断 + 任务规划策略输入
```
...
|fleet_operation_status|车队运维状态（兼容）|旧版摘要字段，当前主流程以当前任务和待执行队列为准|
|current_order_id|当前服务订单编号|判断是否正在服务履约|
|current_task_id|当前任务编号|判断是否正在执行投放、运维、行驶等任务|

### 3.2 健康事实层

以下字段只用于健康事实或兼容摘要，不直接替代任务单状态。

|字段|含义|建议值|
|---|---|---|
|cleanliness_status|清洁事实摘要|兼容或展示字段，不替代 CleaningTask|
|battery_operation_status|电量事实摘要|兼容或展示字段，不替代 ChargingTask|
|maintenance_status|维修事实摘要|兼容或展示字段，不替代 MaintenanceTask|
|failure_status|故障事实摘要|兼容或展示字段，不替代 FailureHandlingTask|
|retirement_status|退役事实摘要|兼容或展示字段，不替代 RetirementTask|
|operation_blocking_reason|不可运营原因|从可运营变为不可运营时记录原因|
|pending_task_queue|待执行任务队列|包含任务类型、任务编号、优先级、队列序号|
...
getRobotaxiTaskPlanningInput(robotaxi, context)
evaluateTaskPlanning(robotaxi, requestedTask, context)
...
|`getRobotaxiTaskPlanningInput`|汇总运营状态、当前任务、队列、位置和资产事实|
|`evaluateTaskPlanning`|任务规划策略执行入口，决定拒绝、立即执行、排队或重排|
|`createFleetOperationTaskIfNeeded`|任务创建入口，避免各模块重复创建|
...
|维修任务完成|当前阶段直接恢复可运营，未来可配置准入复核|无 / ReadinessCheckTask|
|故障处理完成|当前阶段直接恢复可运营，必要时维修或退役|无 / MaintenanceTask / RetirementTask|
...
|待命中|立即执行|立即执行|立即执行|
|已有运维任务中|进入任务规划策略判断|进入任务规划策略判断|进入任务规划策略判断|

第一阶段建议默认规则：

1. 服务履约中，非严重问题等待当前服务完成；
2. 投放行驶中，低电和故障可以中断投放；
3. 已存在待执行任务时，由任务规划策略决定拒绝、排队、重排或升级；
4. 更高严重级别任务可以通过任务规划策略调整队列顺序；
5. 所有等待、排队、拒绝和中断决策都记录业务发生时间和原因。
...
```text
RobotaxiService
  ↓ 汇总综合状态、当前位置、当前任务和待执行队列
  ↓ evaluateTaskPlanning / createFleetOperationTaskIfNeeded
...
|MVP|支持任务规划策略：拒绝、立即执行、排队、重排|
|MVP|任务完成后统一更新 Robotaxi availability_status、当前任务和待执行队列|
