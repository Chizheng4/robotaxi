...
CleaningTask 不直接判断 Robotaxi 是否需要清洁，也不依赖 `needs_cleaning` 这类旧标记作为主流程来源。

清洁需求应由 `RobotaxiService.needsCleaning(...)` 或人工操作触发，再由 CleaningTaskService 创建任务：

```text
Robotaxi 综合状态 + 任务规划策略判断可创建清洁任务
  ↓
...
|READY_TO_CLEAN|Worker 已分配，等待开始清洁|开始清洁|
|CLEANING|清洁中|清洁完成|
|COMPLETED|清洁完成|无|
|CANCELLED|任务取消|无|
|FAILED|任务失败|人工处理|
...
```text
Robotaxi 综合状态 + 任务规划策略允许清洁
  ↓
创建 CleaningTask
  ↓
如 Robotaxi 正在执行更高优先级任务：WAITING_ROBOTAXI_AVAILABLE
  ↓
任务被接管后 Robotaxi 进入运维中，不再参与订单匹配和投放匹配
  ↓
...
|阶段|运营状态|当前任务 / 队列|说明|
|---|---|---|---|
|清洁任务排队|保持当前运营状态|进入 pending_task_queue|排队任务不占用当前任务|
|清洁任务接管|运维中|current_task = CleaningTask|不可参与订单和投放匹配|
|前往清洁中心|运维中|关联 RouteExecution|通过 RouteExecution 行驶|
|清洁中|运维中|CleaningTask = CLEANING|作业执行中|
|清洁完成|可运营|按队列触发下一任务或空闲|当前阶段直接恢复可运营|

当前阶段：清洁完成后直接恢复可运营，不额外进入准入复核。
