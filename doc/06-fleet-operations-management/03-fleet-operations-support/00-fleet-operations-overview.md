# Fleet Operations Support：运维支持闭环总览

## 1. 系统定位

Fleet Operations Support 是 Robotaxi 运营阶段的车队运营保障闭环，核心职责是让已经进入运营阶段的 Robotaxi 在真实运营过程中持续恢复到可运营状态。

```text
Fleet Operations Support = 保证运营阶段 Robotaxi 清洁、补能、维修、故障处理和退出的业务闭环
```
...
|长期供给来源、生产、供应商、车主网络接入|供应管理|
|运营准入和可运营供给形成|供应管理|
|短期投放、再平衡、待命分布|运营管理 / 供需投放管理|
|客户订单匹配|Dispatch / Demand Order|
|客户服务履约|Service Order / Trip|
|经营指标计算|Operating Metrics|

DeploymentTask 属于运营管理下的供需投放管理。它处理可运营 Robotaxi 在服务网络中的位置分布，不处理 Robotaxi 健康恢复。
...
7. 运维完成后更新 Robotaxi 可运营状态；维修和故障处理完成后当前默认直接恢复可运营，未来可通过配置接入准入复核。
...
供应管理决定可运营车辆如何形成
Fleet Deployment 决定这些车辆如何投放和再平衡
