...
维修任务通常比清洁和充电风险更高。维修完成后，Robotaxi 应进入运营准入检查，而不是直接恢复可运营。
...
MaintenanceTask 执行等待、目的地分配、行驶、维修作业
  ↓
RobotaxiService 根据维修结果进入准入、故障阻塞或退役
```
...
|READY_TO_START|准备维修|开始维修|
|IN_PROGRESS|维修中|提交维修结果|
|WAITING_RESULT_CONFIRMATION|等待维修结果确认|确认结果|
|COMPLETED|维修完成|触发准入检查|
|CANCELLED|任务取消|无|
...
如当前服务可完成：等待服务完成
  ↓
Robotaxi 变为 UNAVAILABLE / NEED_MAINTENANCE
  ↓
...
维修结果确认
  ↓
维修成功：进入 PENDING_INSPECTION / READY_FOR_INSPECTION
维修失败：进入 BROKEN 或转 RetirementTask
```
...
|---|---|---|
|维修需求触发但仍在服务|AVAILABLE|WAITING_SERVICE_COMPLETION|
|待维修|UNAVAILABLE|NEED_MAINTENANCE|
|前往维修点|UNAVAILABLE|MOVING_TO_OPS_CENTER|
|维修中|UNAVAILABLE|IN_MAINTENANCE|
|维修成功|PENDING_INSPECTION|READY_FOR_INSPECTION|
|维修失败|UNAVAILABLE|BROKEN|
|不可修复|RETIRED|RETIRED|
...
|requires_readiness_check|是否需要准入复核，默认 true|
