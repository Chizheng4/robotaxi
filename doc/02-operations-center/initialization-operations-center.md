...
```text
02-operations-center/01-robotaxi.md
02-operations-center/02-opscenter.md
02-operations-center/03-field-dictionary.md
01-space-model/initialization-map.md
```

本文档只描述 OpsCenter 与 Robotaxi 的初始化数据、生成规则和校验规则。

---

## 2. 初始化目标

第一版初始化一个统一运营中心，并生成 20 台 Robotaxi。
...
- `OC-001` 位于 Map 右下角空白区域；

- OpsCenter 覆盖 Cell 不得与 Place 覆盖 Cell 重叠；
...
3. OpsCenter 覆盖 Cell 不得与 Place 覆盖 Cell 重叠；
    
4. OpsCenter 容量必须大于等于初始化 Robotaxi 数量；
    
5. OpsCenter 至少具备接收、停放、检查、投放能力；
    
6. OpsCenter 关联的 ServiceArea 必须存在；
    
7. OpsCenter 关联的 ServiceArea 必须位于运营中心附近的接入道路。
...
7. 不生成 Task、Demand、Order、Dispatch、Trip、Metric。
...
```text
initialization-operations-center.md 只定义运营中心与 Robotaxi 的初始数据
不定义具体任务过程
```
