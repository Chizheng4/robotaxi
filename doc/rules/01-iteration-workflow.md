...
版本严格串行不等于 Task 之间同步等待。跨 Task 协作采用事件驱动交接，不得用持续 `wait`、轮询或重复读取状态代替目标 Task 的独立执行。
...
交接成功后，源 Task 应结束当前回合；目标 Task 独立执行，并在里程碑完成、出现真实阻断或任务完成时主动回传不超过 20 行的事实检查点。状态查询只允许按需获取一次即时快照，不得进入持续等待；“等待上游”或 `blocked` 表示报告现状和所需输入后结束回合，不得继续调用等待工具消耗资源。

同一修改在同一时刻只能有一个 owner。跨项目协作只交付边界合同、事实引用和验收要求，不得由同一 Task 跨仓库双写实现；目标项目的责任 Task 必须依据自身 `AGENTS.md` 和项目事实源落地。
