...
`SupplyPlan` 是将一条需求预测结果转成可执行生产安排的独立业务单据。它回答目标区域缺多少 Robotaxi、建议生产多少、何时开始以及预计何时完成。生产计划不创建车辆资产。

```mermaid
flowchart LR
  A["需求预测结果"] -->|生成生产计划| B["生产计划"]
  B -->|确认计划| C["已确认"]
  C -->|创建生产批次| D["生产批次"]
```
...
|建议生产数量|需求预测结果 `recommended_production_quantity`|
|预测期内可形成供给|需求预测结果 `feasible_supply_quantity`|
|生产完成日期|需求预测结果 `full_supply_completion_date`|
|产能约束|生产画像 `supply_production_profile_id`|

建议生产数量表示完整覆盖缺口所需数量；预测期内可形成供给表示受提前期、产能与交付能力约束后，在预测期内能够完成的数量。两者不得混为一个字段。
