...
1. **范围**
   - 初始化供应生产画像。
   - 初始化长期需求预测策略。
   - 实现长期需求预测策略执行服务，生成预测执行和预测结果。
   - 前端“预测策略”行操作提供“执行”。
   - 预测结果作为后续车队生产计划的上游输入，本轮不创建生产计划。

2. **业务边界**
   - 需求画像仍是需求输入事实。
   - 供应生产画像只描述自有生产能力，不代表生产计划或生产执行。
   - 预测策略执行只产生执行记录和预测结果，不创建 Robotaxi、不创建生产批次、不接入模拟运行主路径。

3. **验证**
   - 旧快照缺少新增集合时必须恢复为空或默认集合，不白屏。
   - 执行策略后应生成一条预测执行和至少一条预测结果。
   - 字段展示不得暴露英文内部字段。
   - 提交前检查和真实浏览器加载验证必须通过。
