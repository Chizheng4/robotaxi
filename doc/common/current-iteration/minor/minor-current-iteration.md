...
### v041.1.5 经营规划菜单小版本（执行中）

1. **菜单范围**
   - 新增一级菜单“经营规划”，放在“财务管理”之后。
   - “需求画像”移动到经营规划下作为二级菜单。
   - 新增“供应生产画像”二级菜单。
   - 新增“需求预测”二级菜单，下设“预测策略 / 预测执行 / 预测结果”三级菜单。
   - 供应管理保留供应计划、供给单、运营准入、车商供应、车主供应等供应执行入口。

2. **工程边界**
   - 本轮只做菜单、空集合占位、页面配置和字段字典。
   - 不实现长期需求预测策略执行服务。
   - 不接入模拟运行主路径。

3. **后续大版本约束**
   - 后续经营规划大版本进入业务闭环实现前，必须先制定大版本计划和任务清单并提交规划版本。
   - 大版本计划必须覆盖对象、状态机、服务边界、前端展示、字段字典、验证合同和模拟运行边界，防止遗漏。

### v041.1 经营规划与供给执行框架（执行中）

1. **统一对象**
   - 需求画像主事实收敛为 `DemandProfile`。
   - 通过 `target_object_type / target_object_id / target_object_name` 关联 Place、ServiceArea、Zone。
   - `profile_type` 不再作为主展示和主数据口径，只保留旧数据兼容。

2. **运行态兼容**
   - 新初始化生成统一 `demandProfiles`。
   - 旧快照如仍包含 `placeDemandProfiles / serviceAreaDemandProfiles / zoneDemandProfiles`，恢复时迁移到统一对象。
   - 旧隐藏页面需要数据时，从统一 `demandProfiles` 拆分得到兼容数组。

3. **前端展示**
   - 经营规划口径下的“需求画像”页面展示统一对象字段。
   - 关键数字必须在表格前列展示，包括潜在需求、预计 Robotaxi 需求、峰值需求、服务容量、等待容量、周转能力、供给需求评分，以及可配置输入数字。
   - 需求画像必须支持配置功能，并遵循当前表格行操作和右侧详情表达规则。
   - 不增加新的散列画像菜单，不把画像拆回三个页面。

4. **Zone 汇总**
   - Zone 类型 DemandProfile 的需求字段由包含的 Place 画像汇总。
   - Zone 类型 DemandProfile 的服务容量、等待容量和周转能力由包含的 ServiceArea 画像汇总。
   - Zone 表单只配置区域修正系数，不能配置汇总字段。

5. **边界**
   - 本轮以经营规划和自有生产供给主路径为设计边界。
   - 不提前实现长期需求预测运行、生产计划、生产批次、交付单等代码闭环。
   - 不接入模拟运行主路径，避免影响现有模拟效率和业务闭环。

6. **v041.1.4 经营规划与供应执行文档收敛（执行中）**
   - `DemandProfile` 正式归入 `doc/00-business-planning/02-demand-profile-design.md`。
   - 经营规划层明确为经营目标、需求画像、供给生产画像、长期需求预测。
   - 供应管理层明确为自有生产执行主路径：车队生产计划、生产批次、车队分配策略、Robotaxi 交付单、运营准入任务。
   - 当前供应不设计车商供应主线；车主网络供给作为未来快速供给路径单独设计。
   - 车队分配策略输出按 Zone / 运营中心记录 `allocated_quantity` 与 `allocated_robotaxi_ids`，不再表达为某个 Robotaxi 分配多辆车。
   - 生产批次完成后调用 Robotaxi 服务创建 N 台 Robotaxi；Robotaxi 仍是独立服务化资产对象。
   - Robotaxi 交付单是一张物流批次单，可包含多台 Robotaxi，交付完成后逐车触发运营准入任务。
   - 本轮只更新方案设计和当前迭代记录，不进入代码实现，不接入模拟运行。

7. **v041.1.1 配置保存整体重算补丁（已完成）**
   - 修复需求画像配置保存后只重算 ServiceArea / Zone、未先重算 Place 的问题。
   - 统一重算顺序为 Place -> ServiceArea -> Zone，确保地点、服务区域和区域画像随任意配置变化实时联动。
   - 旧画像迁移时不再保留过期自动计算字段，ServiceArea 缺少容量配置时从基础对象补齐后再参与 Zone 汇总。
   - `verify-v041-1-demand-profile-unified.mjs` 已补充旧数据迁移、ServiceArea 配置变更和 Zone 联动重算合同。

8. **v041.1.2 计算时间边界补丁（已完成）**
   - 修复需求画像人工配置保存后使用模拟时间占位作为计算时间的问题。
   - 业务初始化和人工保存默认记录真实计算时间；未来模拟触发必须显式传入模拟时间上下文。
   - 归一化展示保留已有计算时间，避免列表渲染导致计算时间变化。
   - `verify-v041-1-demand-profile-unified.mjs` 已补充真实计算时间和模拟时间显式传入合同。

9. **v041.1.3 设计文档收敛补丁（已完成）**
   - 将需求画像正式设计文档收敛为 `doc/02-operation-zone/04-demand-profile-design.md`。
   - 删除 `doc/02-operation-zone/05-calculation.md` 独立计算草稿，计算逻辑集中写入正式设计文档。
   - 正式文档对齐当前实现：统一 `DemandProfile`、配置保存整体重算、Zone 汇总、旧数据兼容、真实计算时间与未来模拟时间边界。
   - 修正空间模型升级文档中的旧名称引用，避免继续指向旧“空间经营画像方案”。
   - 该路径在 v041.1.4 被经营规划层新结构承接，当前正式路径为 `doc/00-business-planning/02-demand-profile-design.md`。
...
1. **供应管理菜单与未来对象接入**
   - 一级菜单新增“供应管理”。
   - 本段为 v040.34 菜单期旧设计记录，已被 v041.1.4 经营规划 / 供应执行新框架调整。
   - 当前口径：需求画像、需求预测属于经营规划；供应管理聚焦自有生产、车队分配、交付和运营准入。
   - 当前仅运营准入具备完整业务闭环；新增经营规划与供应执行对象作为未来服务化对象设计，不进入模拟运行主路径。
...
4. **空间画像对象升级**
   - 地点、服务区和区域画像不再作为地图空间菜单散列入口，统一通过经营规划口径的“需求画像”查看。
