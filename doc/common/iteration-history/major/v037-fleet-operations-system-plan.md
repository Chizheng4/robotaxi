...
- 在运营中心管理 / 任务单管理下接入清洁、充电、维修、故障处理和退役任务页面；
- 新增五类任务的表格配置、对象映射、状态字段、详情集合和运行态持久化；
- 新增 `scripts/verify-v037-3-fleet-operation-pages.mjs` 验证菜单、表格、状态、运行态和快照接入；
- 修改 `src/main.jsx` 后已重新生成 `src/main.bundle.js`；
- 已通过 bundle 语法检查、字段展示合同、v037.2 服务合同和 v037.3 页面接入验证；
- 已通过 HTTP 入口和 bundle 资源 200 检查。
...
- `scripts/check-before-commit.sh` 已纳入 v037.2 服务合同验证和 v037.3 页面接入验证；
- 已执行字段展示合同、v037.2 服务合同、v037.3 页面接入验证；
- 已重新生成 `src/main.bundle.js` 并完成 bundle 语法检查；
- 已完成本地 HTTP 入口和 bundle 资源检查；
- v037 计划完成后归档到历史大版本目录。
