...
运营中控台支持真实地理地图与模拟网格双模式。广州地理地图可以直接规划运营区域、地点和服务区域边界，经过校验与影响预览后发布为版本化地理投影；原有 Cell、Road、Route、Robotaxi 和模拟运行始终保留，视图切换或发布空间方案不会重置业务状态。当前真实地理模式用于空间经营建模和路径可视化，不代表自动驾驶导航能力。
