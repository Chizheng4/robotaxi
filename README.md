...
运营中控台提供相互隔离的“城市地理”和“网格仿真”空间场景。城市地理使用广州原生空间目录规划最多两级的运营区域、地点和服务区域，通过真实几何包含、底图要素参考和版本化方案生成正式城市空间对象，并按地图缩放层级逐步显示 Zone、地点和服务区域；网格仿真继续承载当前 Cell、Road、Route、Robotaxi 和模拟运行。两者复用统一对象与能力接口，但不共享空间事实、道路、位置、路线或运行快照；切换查看场景和发布城市运营区域方案不会改变业务运行场景。当前城市地理用于空间经营建模，尚未启用城市道路路由和 Robotaxi 地理位置，不代表自动驾驶导航能力。
